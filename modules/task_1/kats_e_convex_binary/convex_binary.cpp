...
std::vector<int*> MainClass::jarvis(int** convex_arr, int n) {
	...
	if (n == 1) {
		...
		while (1) {
			mincos = 2;
			for (int i = 0; i < n; i++) {
				cos = round(sc.cosinus(last, beforelast, convex_arr[i]) * 10000000) / 10000000;
				if (cos < mincos) {
					minind = i;
					mincos = cos;
...
std::vector<int*> MainClass::getComponent(int** arr, int n, int m, int** *convex_arr, int *num) {
	MainClass mc;
	int x = 0, y = 0, bf = 1;
	int A, B, C;
	for (int i = 0; i < n; i++)
...
std::vector<int*> MainClass::inside(std::vector<int*> root, int** convex_arr, int n) {
